...
const pct = (v) => (v == null ? "â€”" : (Number(v) * 100).toFixed(1) + "%");
const pct100 = (v) => (v == null ? "â€”" : Number(v).toFixed(1) + "%");
...
function lineChart(host, points) {
  ...
  const marks = points
    .map((p, i) => {
      ...
      const q = p.quality == null ? "â€”" : pct(p.quality);
      return `<g>
...
function renderKPIs(s) {
  ...
  host.innerHTML = `
    <div class="kpi"><div class="label">Cost vs all-flagship</div>
      <div class="value" style="color:var(--router)">âˆ’${pct100(c.cost_saving_vs_flagship_pct)}</div>
    ...
    <div class="kpi"><div class="label">p50 latency</div>
      <div class="value">âˆ’${pct100(c.latency_p50_change_pct)}</div>
    ...
    <tbody>${feed
      .map((f) => {
        ...
        const tail = climbed
          ...
          : "";
        return `<tr>
          <td>${esc(f.prompt.slice(0, 64))}${f.prompt.length > 64 ? "â€¦" : ""}
